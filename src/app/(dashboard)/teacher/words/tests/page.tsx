...
import { getTeacherWordTestAssignments } from '../actions'
...
export default async function TeacherWordTestsPage() {
  const user = await getCurrentUser()
  if (!user || user.role !== 'TEACHER' || !user.academyId) redirect('/login')

  const assignments = await getTeacherWordTestAssignments()
            ...
            {assignments.map((a) => {
              ...
              return (
                <div key={a.id} className="flex items-center gap-3 px-4 py-3">
                  <Link
                    href={`/teacher/words/sets/${a.wordSet.id}/test/${a.id}/results`}
                  ...
                  <Link href={`/teacher/words/tests/${a.id}/edit`}>
                    <button
                      type="button"
