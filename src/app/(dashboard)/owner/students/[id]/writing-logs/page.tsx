...
import { ArrowLeft, PenLine, AlertCircle, CheckCircle2 } from 'lucide-react'
...
export default async function OwnerWritingLogsPage({ params }: { params: { id: string } }) {
  const user = await getCurrentUser()
  if (!user || user.role !== 'ACADEMY_OWNER' || !user.academyId) redirect('/login')

  const student = await prisma.student.findFirst({
    where: { id: params.id, user: { academyId: user.academyId, isDeleted: false } },
    include: {
