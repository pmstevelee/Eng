...
import { GradeClient } from '@/app/(dashboard)/teacher/tests/[testId]/grade/_components/grade-client'

export default async function OwnerTestGradePage({
  params,
}: {
  params: { id: string }
}) {
  ...
  if (user.role !== 'ACADEMY_OWNER') redirect('/login')
    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mb-6">
        <div className="mb-1 flex items-center gap-2">
          <a href={`/owner/tests/${testId}`} className="text-sm text-gray-500 hover:text-gray-700">
            {test.title}
          </a>
          <span className="text-gray-400">/</span>
          <span className="text-sm text-gray-700">채점</span>
        </div>
