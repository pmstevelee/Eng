...
import { withdrawAcademy } from '../actions'

interface WithdrawModalProps {
  academyName: string
}

export function WithdrawModal({ academyName }: WithdrawModalProps) {
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result = await withdrawAcademy(formData)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl border border-gray-200 p-6 w-full max-w-md shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-base font-bold text-gray-900">학원 탈퇴 확인</h2>
            </div>

            <div className="mb-5">
              <p className="text-sm text-gray-700 mb-3">
                <strong>{academyName}</strong> 학원을 탈퇴하면 다음 데이터가 모두 영구 삭제됩니다:
              </p>
