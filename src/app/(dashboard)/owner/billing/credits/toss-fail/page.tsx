...
export default async function CreditsTossFailPage({ searchParams }: PageProps) {
    ...
    <div className="flex min-h-[60vh] flex-col items-center justify-center px-4 py-16">
      <div className="w-full max-w-md text-center">
        ...
        <h1 className="mb-3 text-2xl font-bold text-gray-900">
          {isCanceled ? '결제가 취소되었습니다' : '크레딧 결제에 실패했습니다'}
        </h1>

        <p className="mb-2 text-gray-500">
          {isCanceled ? '결제창을 닫으셨습니다. 다시 시도하려면 아래 버튼을 눌러주세요.' : message}
        </p>

        {!isCanceled && <p className="mb-8 text-sm text-gray-400">오류 코드: {code}</p>}
