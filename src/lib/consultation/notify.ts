...
/** 상담 예약 확정/전날 리마인드 알림 — 예약·문의 상태를 다시 확인하고 발송 */
export async function notifyAppointment(appointmentId: string, kind: AppointmentKind): Promise<SendNotificationResult | null> {
  ...
  // 취소·완료된 예약, 등록·이탈 처리된 문의에는 보내지 않음
  ...
  const base = {
    학원명: academyDisplayName(lead.academy),
  ...
  return kind === 'confirmed'
    ? sendNotification({
        ...
        variables: { ...base, 학원연락처: academyPhone(lead.academy) },
