...
export async function getStudentWordStats(
  ...
  // 자율학습 단어시험 + 교사 출제 단어시험을 문항 수 가중 평균으로 합산
  const testTally = new Map<string, { correct: number; total: number; count: number }>()
  for (const agg of [selfTestAgg, assignedTestAgg]) {
    for (const r of agg) {
      const t = testTally.get(r.studentId) ?? { correct: 0, total: 0, count: 0 }
      t.correct += r._sum.score ?? 0
      t.total += r._sum.totalQuestions ?? 0
  ...
  testTally.forEach((t, studentId) => {
    ...
    stat.testAccuracy = t.total > 0 ? Math.round((t.correct / t.total) * 100) : null
  })
