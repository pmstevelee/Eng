/**
 * prisma/seed-partb4.ts
 * Book 1 - Part B4 공용 문제 시드 (Tenses - Passives, Intermediate / Upper-Intermediate)
 *
 * 출처: partb4.pdf
 * - B4: Intermediate / Upper-Intermediate, 시제·수동태 (152문제)
 *
 * 모두 공용 문제(academyId=null)로 등록.
 *
 * 실행: npm run seed:partb4
 */
...
const SOURCE_TAG = 'partb4'

async function main() {
  console.log('📚 Part B4 공용 문제 시드 시작...\n')

  const dataPath = path.join(__dirname, 'data-partb4.json')
  const raw = fs.readFileSync(dataPath, 'utf-8')
  const data = JSON.parse(raw) as AllData

  const section = data.B4
  console.log(`── B4 (${section.label}) ─────────────`)
  console.log(`   대상 문제: ${section.questions.length}개`)
  console.log(`   영역: GRAMMAR / 시제·수동태 (중급)`)
  console.log(`   CEFR: B1 상 (난이도 7)`)
