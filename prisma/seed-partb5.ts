/**
 * prisma/seed-partb5.ts
 * Book 1 - Part B5 공용 문제 시드 (Clauses, Elementary / Pre-Intermediate)
 *
 * 출처: partb5.pdf
 * - B5: Elementary / Pre-Intermediate, 절 (adjective/adverb/noun clauses) (180문제)
 *
 * 모두 공용 문제(academyId=null)로 등록.
 *
 * 실행: npm run seed:partb5
 */
...
const SOURCE_TAG = 'partb5'

async function main() {
  console.log('📚 Part B5 공용 문제 시드 시작...\n')

  const dataPath = path.join(__dirname, 'data-partb5.json')
  const raw = fs.readFileSync(dataPath, 'utf-8')
  const data = JSON.parse(raw) as AllData

  const section = data.B5
  console.log(`── B5 (${section.label}) ─────────────`)
  console.log(`   대상 문제: ${section.questions.length}개`)
  console.log(`   영역: GRAMMAR / 절 (adjective/adverb/noun clauses)`)
  console.log(`   CEFR: A2 상 (난이도 4)`)
  ...
  for (const q of section.questions) {
    ...
    await prisma.question.create({
      data: {
        ...
        subCategory: '절 (adjective/adverb/noun clauses)',
        difficulty: 4,
        cefrLevel: 'A2 상',
